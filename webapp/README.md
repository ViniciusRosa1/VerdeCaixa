...
1. Copie `api/.env.example` para `api/.env` e preencha `ADMIN_PASSWORD`, `AUTH_ACCESS_SECRET`, `AUTH_REFRESH_SECRET` e `RESEND_API_KEY`. Para enviar de um endereço próprio, verifique o domínio no Resend e ajuste `RESEND_FROM`.
2. Inicie o PostgreSQL, aplique a migration com `npm --prefix api run db:deploy` e carregue a demonstração com `npm --prefix api run db:seed`.

...
const openapiBaseUrl = baseUrl.endsWith('/api/v1') ? baseUrl.slice(0, -7) : baseUrl;
export const openapiClient = createClient<paths>({ baseUrl: openapiBaseUrl, credentials: 'include' });
let csrfToken: string | null = null;
let refreshPromise: Promise<boolean> | null = null;

export class ApiClientError extends Error {
  constructor(public readonly status: number, public readonly payload: ApiError) { super(payload.message); }
}
...
async function refresh() {
  refreshPromise ??= (async () => {
    ...
    const response = await fetch(`${baseUrl}/auth/refresh`, { method: 'POST', credentials: 'include', headers: { 'x-csrf-token': token } });
    return response.ok;
...
export async function api<T>(path: string, init: RequestInit = {}, retry = true): Promise<T> {
  ...
  if (!response.ok) {
    const payload = await response.json().catch(() => ({ code: 'HTTP_ERROR', message: 'Não foi possível concluir a solicitação.', statusCode: response.status, timestamp: new Date().toISOString() })) as ApiError;

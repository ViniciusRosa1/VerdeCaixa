...
function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const isPublicPage = pathname === "/login" || pathname === "/criar-conta";
  useEffect(() => {
    api<ApiUser>("/auth/me")
      .then(setUser)
      .catch(() => setUser(null))
  ...
  useEffect(() => {
    ...
    if (!loading && user && isPublicPage) router.replace("/dashboard");
  }, [loading, isPublicPage, router, user]);
  const value = {
    user,
    loading,
  ...
  return (
    <AuthContext.Provider value={value}>
      {loading && !isPublicPage ? (
        <div className="grid min-h-screen place-items-center bg-[#F6FAF3] text-sm text-[#60705E]">

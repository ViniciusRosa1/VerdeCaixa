...
import { useSaveSupplier, useSupplier } from "@/lib/api/entity-hooks";
import { phoneDigits } from "@/lib/phone";

export function FornecedoresForm({ id }: { id?: string }) {
  const router = useRouter();
  const detail = useSupplier(id);
  const save = useSaveSupplier(id);
  const [message, setMessage] = useState("");
  if (id && detail.isLoading) return <p>Carregando fornecedor…</p>;
  ...
  return (
    <FormPage
      title={`${id ? "Editar" : "Novo"} fornecedor`}

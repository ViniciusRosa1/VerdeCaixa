  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = id
      ...
      : {
          ...
          recurrenceEndsOn:
            plan === "RECURRING"
              ? optional(form, "recurrenceEndsOn")
              : undefined,
          notes: optional(form, "notes"),
        };
    try {
  ...
  return (
    <div className="page-enter mx-auto max-w-4xl">
      <PageHeading
        ...
        description="Registre os valores previstos para entrada."
      />
      {message && (
        <p
          role="alert"
          className="mb-4 rounded-xl bg-surface p-3 text-sm text-danger"
        >
          {message}
        </p>
      )}
      <form onSubmit={submit} className="space-y-4">
        <Card>
          <CardContent className="grid gap-5 sm:grid-cols-2">
            ...
            <Field label="Cliente" htmlFor="counterpartyId">
              <Select
                name="counterpartyId"
                required
                defaultValue={current?.counterparty.id}
              >
            ...
            <Field label="Projeto" htmlFor="projectId">
              <Select
                name="projectId"
                defaultValue={current?.project?.id ?? "none"}
              >
        ...
        {!id && (
          <Card>
            <CardContent className="grid gap-5 sm:grid-cols-2">
              <Field label="Forma do lançamento" htmlFor="plan">
              ...
              {plan === "RECURRING" && (
                <Field label="Término opcional" htmlFor="recurrenceEndsOn">
                  <Input
                    id="recurrenceEndsOn"

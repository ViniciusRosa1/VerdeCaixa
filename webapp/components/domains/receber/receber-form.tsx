...
import { useAccounts, useCategories, useClients, useEntry, useProjects, useSaveEntry } from "@/lib/api/hooks";

export function ReceberForm({ id }: { id?: string }) {
  ...
  if (id && detail.isLoading) return <p>Carregando receita…</p>;
  ...
  return <div className="page-enter mx-auto max-w-4xl">
    <PageHeading eyebrow="Financeiro" title={id ? "Editar receita" : "Nova receita"} description="Registre os valores previstos para entrada." />
    ...
    <form onSubmit={submit} className="space-y-4">
      <Card><CardContent className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Cliente" htmlFor="counterpartyId"><Select name="counterpartyId" required defaultValue={current?.counterparty.id}>{(clients.data?.data ?? []).map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</Select></Field>
        <Field label="Categoria" htmlFor="categoryId"><Select name="categoryId" required defaultValue={current?.category.id}>{(categories.data?.data ?? []).filter((item) => item.kind === "INCOME").map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</Select></Field>
      ...
      <Card><CardContent><Field label="Observações" htmlFor="notes"><Textarea id="notes" name="notes" defaultValue={current?.notes} /></Field></CardContent><CardFooter className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => router.push("/receber")}>Cancelar</Button><Button disabled={save.isPending}>{save.isPending ? "Salvando…" : "Salvar receita"}</Button></CardFooter></Card>
    </form>
  </div>;
}

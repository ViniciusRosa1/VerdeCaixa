...
export class AuthController {
  ...
  async refresh(
    ...
    const result = await this.auth.refresh(
      request.cookies?.vc_refresh as string | undefined,
    );
    this.setSessionCookies(response, result.access, result.refresh);
    return { user: result.user };
  }
  ...
  private setSessionCookies(
    ...
    response.cookie("vc_access", access, {
      ...this.cookie(true, "/api"),
      maxAge: 15 * 60_000,
    });
    response.cookie("vc_refresh", refresh, {
      ...this.cookie(true, "/api/v1/auth"),
      maxAge: 7 * 86_400_000,
    });
  }
